...
class model(object):
    ...
    def start_index(self, stem):
        ...
        i = 0
        while i < int(s):
            index_element = IndexElement(i, self.corpus_path, self.posting_and_dictionary_path, stem, 400, stop_words)
            tasks.append(index_element)
            i += 1
        if files_number % 400 > 0:
            tasks.append(IndexElement(i, self.corpus_path, self.posting_and_dictionary_path, stem, files_number % 400, stop_words))
        starttime = time.time()
